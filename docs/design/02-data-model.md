...
| **項目を外しても、列の値は消さない。**外した項目の定義はサーバが保管し、**同じ列名で戻せば元の定義(型・参照先)が復活する**(違う型では戻せない)。テーブルの削除も論理削除で、レコードとビューは残る(`restore` で戻る)。**削除済みと同じ列名のテーブルは作れない**(戻すか別名に)。初めから入っている 5 つ(`system`)は削除できない | 画面は確認を挟まず「元に戻す」で守る(05 §5)。戻せるためには、消してはいけない。列の値を別の型として読ませない |
