...
| ID | 視点 | 段階 | 性質 | 層 | ケース | 対応する資産 | API の資産 |
|---|---|---|---|---|---|---|---|
| REC-020 | 利用者 | 一覧 | キー | L3 | 利用者: 3 で商談へ(ビューは切り替わらない)、V → 2 でカンバン、V → 3 でレポートへ切り替わる | `smoke.mjs「3 で商談へ移動(ビューは切り替わらない)」` / `smoke.mjs「V → 3 でレポートへ切り替わる」` | — |
...
| REC-025 | 利用者 | 一覧 | 整合 | L3 | 利用者: サイドバーのテーブルをドラッグで並べ替え → 1 が並べ替え後の先頭へ。再読み込みしても残る。ゆっくり押しても(動かさなければ)1 回で開く | `smoke.mjs「サイドバーのテーブルをドラッグで並べ替えられる」` / `smoke.mjs「1 は並べ替え後の先頭へ」` / `smoke.mjs「サイドバーの行はゆっくり押しても 1 回で開く」` / `smoke.mjs「並びは保存される」` | — |
...
| REC-028 | 利用者 | 一覧 | 整合 | L3 | 利用者: 見出しの右端をつまんで動かす → その列の幅が変わり、右の列がずれる。再読み込みしても残る(ビューに保存)。右端をダブルクリック → その列の中身に合わせる(切れていたものが全部見える) | `smoke.mjs「見出しの右端をつまむと列の幅が変わり、右の列がずれる」` / `smoke.mjs「列の幅はビューに保存される(再読み込み後も残る)」` / `smoke.mjs「ダブルクリックで中身に合わせる(切れていた日付が全部見える)」` | — |
| REC-029 | 利用者 | 一覧 | 整合 | L1 | `columnWidth`: 無い・数でない幅は既定の 160、範囲(60〜1200)の外は端、端数は丸める。`withColumnWidth` は指した列の幅だけを変える | `columns.test.ts` | — |
| REC-030 | 利用者 | 一覧 | 表記 | L3 | 利用者: 日付の列を最小(60px)まで狭める → 日付は列の中で … で切れ、隣の列へはみ出さない(J-042) | `smoke.mjs「最小まで狭めても、日付は列の中で切れる(隣の列へはみ出さない)」` | — |
...
| REC-062 | システム | 作成 | 整合 | L2 | `insert`: 必須の選択肢は先頭の値、`user` 型は自分、`created_at` と `updated_at` は今。定義に無い列(例 `bogus`)が本文にあれば 400 で何も作らない。`update` も同じく 400 で行は変わらない(04 §11。`readonly` の列は本文にあってよい) | `engine.test.ts` | — |
...
| ID | 視点 | 段階 | 性質 | 層 | ケース | 対応する資産 | API の資産 |
|---|---|---|---|---|---|---|---|
| REC-080 | 外部 | 検証 | 安全弁 | L2 | `insert`: 数値の列に文字 `"12.34"` → 400。`NaN` → 400。数値なら `scale` の桁で丸める | `engine.test.ts` | — |
| REC-081 | 外部 | 検証 | 安全弁 | L2 | `insert`: 日付の列に `2026/09/30` や `2026-13-01` → 400。`2026-09-30` は通る。日時は ISO 8601 だけ | `engine.test.ts` | — |
