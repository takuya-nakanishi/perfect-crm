...
| ACT-002 | システム | 記録 | 整合 | L2 | `insert`(activities)で日付を省く → 今日が入る(必須の検証より先に既定値が効く) | — |
| ACT-003 | システム | 記録 | 整合 | L2 | `insert`(activities)で種別を省く → 先頭の選択肢(電話)。記録者は自分 | — |
...
| ACT-027 | システム | 時系列 | 整合 | L2 | `timeline`: 削除されたテーブル(論理削除)の活動は出ない。戻せば出る | — |
