...
| ACT-005 | システム | 記録 | 整合 | L2 | `insert` / `update` の内容に言及 → `mentions` 列に `テーブル名:ID` の JSON。言及を消せば null。言及した側のレコードだけが変わる(言及先の行は変わらない) | `engine.test.ts` |
...
| ACT-027 | システム | 時系列 | 整合 | L2 | 画面から足したテーブル T のレコード R を関連先にし、取引先 A に言及した活動がある。T を論理削除 → A の `timeline` にその活動が `mention` として残り、`related.name` は R の名前を引ける(参照は保つ。02 §4)。T を戻しても変わらない | `engine.test.ts` |
